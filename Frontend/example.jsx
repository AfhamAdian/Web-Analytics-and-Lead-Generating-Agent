...
import { Globe, TrendingUp, Users, Eye, MousePointer, Calendar, Clock, MapPin, Shield, CheckCircle, XCircle } from 'lucide-react';
...
export default function WebAppDashboard() {
  ...
  const aggregateAnalyticsData = (analyticsResults) => {
    ...
    let totalOsStats = {};
    let totalCookieConsent = { accepted: 0, rejected: 0, notSet: 0 };
              ...
              <>
                ...
                <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-100 w-full">
                  <div className="flex items-center mb-6">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-gray-900">City Analytics</h2>
                      <p className="text-gray-600">
                        {Object.values(dashboardAnalytics.regionStats || {}).reduce((sum, count) => sum + count, 0)} total visitors by region
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    {Object.entries(dashboardAnalytics.regionStats || {})
                      ...
                      .map(([region, count], index) => {
                        ...
                        return (
                          <div key={region} className="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-teal-50 rounded-lg border border-gray-200 hover:shadow-md transition-all duration-200">
                            <div className="flex items-center space-x-4 flex-1">
                              <div className="flex items-center justify-center w-8 h-8 bg-gradient-to-r from-green-600 to-teal-600 text-white rounded-full text-sm font-bold">
                                {index + 1}
                              </div>
                              <div className="flex-1">
                                <span className="font-semibold text-gray-900 text-lg">
                            ...
                            <div className="flex items-center space-x-4 flex-shrink-0">
                              ...
                              <div className="w-24 bg-gray-200 rounded-full h-3">
                                <div
                                  className="h-3 rounded-full bg-gradient-to-r from-green-600 to-teal-600 transition-all duration-500"
                                  style={{ width: `${Math.max(percentage, 5)}%` }}
                                ></div>
                              </div>
                            </div>
                ...
                <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-100 w-full">
                  <div className="flex items-center mb-6">
                    <div className="bg-gradient-to-r from-yellow-600 to-orange-600 rounded-full p-3 mr-4">
                      <Shield className="text-white" size={24} />
                    </div>
                    <div>
                      <h2 className="text-2xl font-bold text-gray-900">Cookie Consent Analytics</h2>
                      <p className="text-gray-600">
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-6 border border-green-200">
                      ...
                      <div className="text-3xl font-bold text-green-600 mb-2">
                        {(dashboardAnalytics.cookieConsent?.accepted || 0).toLocaleString()}
                      </div>
                      <div className="text-sm text-gray-600">
                        {(() => {
                          const total = (dashboardAnalytics.cookieConsent?.accepted || 0) + 
                                       (dashboardAnalytics.cookieConsent?.rejected || 0);
                          return total > 0 ? ((dashboardAnalytics.cookieConsent?.accepted || 0) / total * 100).toFixed(1) : '0.0';
                        })()}% of visitors
                      </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-red-50 to-rose-50 rounded-lg p-6 border border-red-200">
                      ...
                      <div className="text-3xl font-bold text-red-600 mb-2">
                        {(dashboardAnalytics.cookieConsent?.rejected || 0).toLocaleString()}
                      </div>
                      <div className="text-sm text-gray-600">
                        {(() => {
                          const total = (dashboardAnalytics.cookieConsent?.accepted || 0) + 
                                       (dashboardAnalytics.cookieConsent?.rejected || 0);
                          return total > 0 ? ((dashboardAnalytics.cookieConsent?.rejected || 0) / total * 100).toFixed(1) : '0.0';
                        })()}% of visitors
                      </div>
                    </div>
                  </div>
